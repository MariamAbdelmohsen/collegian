...
function giornalismo_setup_theme() {
	register_nav_menus(
      ...
      'newsfooter' => __('FooterMenu', 'giornalismo')
		)
	);
...
if ( ! function_exists ( 'giornalismo_author_byline' ) ) {
	function giornalismo_author_byline()
	{
		if ( esc_attr( get_theme_mod( 'giornalismo-twitter-handle' ) == 1) and ( get_the_author_meta( 'twitter-handle' ) ) ) {
			$html = __( 'By ', 'giornalismo' ) . '<a href="' . esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ) . '">' . get_the_author_meta( 'display_name' ) . '</a>, <a href="' . get_the_author_meta( 'twitter-link' ) . ' target="_blank">' . get_the_author_meta( 'twitter-handle' ) . '</a>, ';
		} else {
			$html = __( 'By ', 'giornalismo' ) . '<a href="' . esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ) . '">' . get_the_author_meta( 'display_name' ) . '</a>, ';
		}
		return $html;
	}
}
...
if ( ! function_exists( 'giornalismo_staff_author_social_area' ) ) {
	...
	function get_excerpt(){
		...
		$excerpt = substr($excerpt, 0, 140); //Edit Here for character count
		$excerpt = substr($excerpt, 0, strripos($excerpt, " "));
		//$excerpt = $excerpt.'... <a href="'.$permalink.'">more</a>';
		return $excerpt;
	}

	function get_CategoryId($title){
		global $wp_query;
