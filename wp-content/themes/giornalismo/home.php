...
<!--					<h5 class="byline"><?php //if ( esc_attr( get_theme_mod( 'giornalismo-author-byline' ) ) == 1 ) { echo giornalismo_author_byline(); } ?><?php //the_date( get_option( 'date_format' ) );?><?php //if ( esc_attr( get_theme_mod( 'giornalismo-post-comments' ) ) == 1 ) { echo ', '; comments_popup_link( __( '0 Comments', 'giornalismo' ), __( '1 Comment', 'giornalismo' ), __( '% Comments', 'giornalismo' ),'', __( 'Comments Off', 'giornalismo' ) ); } ?> </h5> -->
				...
				<?php echo get_excerpt(); //the_excerpt(); ?>
			</article>
			<?php endwhile; endif; wp_reset_postdata(); ?>
			<?php
...
<!--					<h5 class="byline"><?php //if ( esc_attr( get_theme_mod( 'giornalismo-author-byline' ) ) == 1 ) { echo giornalismo_author_byline(); } ?><?php //the_date( get_option( 'date_format' ) );?><?php //if ( esc_attr( get_theme_mod( 'giornalismo-post-comments' ) ) == 1 ) { echo ', '; comments_popup_link( __( '0 Comments', 'giornalismo' ), __( '1 Comment', 'giornalismo' ), __( '% Comments', 'giornalismo' ),'', __( 'Comments Off', 'giornalismo' ) ); } ?></h5>-->
	...
	<!--Begin Column Four-->
	<section class="column-three home-column">
		<?php if ( esc_attr( get_theme_mod( 'giornalismo-column-four-category' ) ) != 'none' ) { $column_four_cat = esc_attr( get_theme_mod( 'giornalismo-column-four-category' ) ); } else { $column_four_cat = ''; } ?>
			<?php if ( esc_attr( get_theme_mod( 'giornalismo-column-four-count' ) ) ) { $column_four_count = esc_attr( get_theme_mod( 'giornalismo-column-four-count' ) ); } else { $column_four_count = 5; } ?>
			<?php if ( ( $column_four_cat != '' ) and ( $column_four_cat != 'none' ) ) {
				$column_four_slug = get_category( $column_four_cat );
				if( function_exists( 'rl_color' ) ) { $category_four_color = 'style="background-color: ' . rl_color( $column_four_cat ) . ';"'; } else { $category_four_color = ''; }?>
				<h5 class="label-head category-<?php echo $column_four_slug->slug; ?>" <?php echo $category_four_color; ?>><a href="<?php echo esc_url( get_category_link( $column_four_cat ) ); ?>" <?php echo $category_four_color; ?>><?php echo get_cat_name( $column_four_cat ); ?></a></h5>
			<?php } ?>
			<?php
				$column_four_args = array(
					'category_name' 	=> get_cat_name( $column_four_cat ),
					'posts_per_page' 	=> intval( $column_four_count ),
					'post__not_in' 		=> $do_not_duplicate,
					'orderby' 			=> 'date',
					'order' 			=> 'DESC'
				);
				$column_four = new WP_Query( $column_four_args );
				if ( $column_four->have_posts() ) : while ( $column_four->have_posts() ) : $column_four->the_post();
				if ( ( $column_four_cat == '' ) or ( $column_four_cat == 'none' ) ) { $margin = 'style="margin-top:0px;margin-bottom:10px;"'; } else { $margin = ''; }
			?>
			<article id="post-<?php the_ID(); ?>" <?php post_class( 'column-three-story column-story clearfix' ); echo $margin; ?>>
				<?php if ( ( $column_four_cat == '' ) or ( $column_four_cat == 'none' ) ) { $cats = get_the_category(); if ( function_exists( 'rl_color' ) ) { $category_four_color = 'style="background-color: ' . rl_color( $cats[ 0 ]->term_id ) . ';"'; } else { $category_four_color = ''; } ?><h5 class="label-head" <?php echo $category_four_color; ?>><a href="<?php echo esc_url( get_category_link( $cats[ 0 ]->term_id ) ); ?>" <?php echo $category_four_color; ?>><?php echo $cats[ 0 ]->cat_name; ?></h5><?php } ?>
				<?php if ( has_post_thumbnail() ) { ?>
					<div class="featured-photo"><a href="<?php the_permalink(); ?>"><?php the_post_thumbnail( 'giornalismo-single' ); ?></a></div>
				<?php } ?>
			...
			<?php endwhile; endif; wp_reset_postdata(); ?>
		<?php
		if ( esc_attr( get_theme_mod( 'giornalismo-column-four-category' ) ) != 'none' ) { ?>
			<a class="read-more-posts-link" <?php echo $category_four_color; ?> href="<?php echo esc_url( get_category_link( $column_four_cat, 'category' ) ); ?>"><?php _e( 'View More &rsaquo;&rsaquo;', 'giornalismo' ); ?></a>
		<?php } else { ?>
			<?php next_posts_link( '<span class="read-more-posts-link">' . __( 'View More &rsaquo;&rsaquo;', 'nuovo' ) . '</span>' ); ?>
		<?php }
		?>
